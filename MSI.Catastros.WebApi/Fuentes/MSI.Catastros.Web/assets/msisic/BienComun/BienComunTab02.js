...
function EditarLoteLinderos(value) {
   ...
   // $('#lindero_fuente_' + value).removeAttr('disabled');
    ...
    if (Number(value) <= 4) {        $('#lindero_medcam_' + value).focus().select();
...
function ActualizarCodigo(element) {
    ...
    if (!valor) {        $('#lindero_colin2_' + num_item).val(element.value);
...
function GrabarLoteLinderos() {
    ...
    $('#LoteLinderos tr').each(function (index, item) {
        ...
        arrayObject.push({
            ...
            "CODUNI": $('#CodigoUnidadCatastral').val()
        });
    });
    return arrayObject;
}
...
function RecuperarLinderos() {
    $.ajax({
        type: "POST", url: ROOT+"BienComun/BuscarLoteLinderos", 
