...
LoteLinderos = $("#LoteLinderos").DataTable({
    "ajax": {
        "url": ROOT + "BienComun/BuscarLoteLinderos",
        "type": "POST",
        "datatype": "json",
        "data": function (d) {
            d.CodUnidadLindero = $('#CodigoUnidadCatastral').val(),
            d.Estatico = Estatico
        }
    },
    "columns": [
...
$("#LoteLinderos tbody").on('click', 'button.btnDeleteLindero', function (event) {
    ...
    var enlace = ROOT + 'BienComun/EliminarLinderoUnidad';
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                $.post(enlace,
                {
                    CodUnidadLindero: $('#CodigoUnidadCatastral').val(),
                    Estatico: Estatico,
...
$("#LoteLinderos tbody").on('click', 'button.btnEditLindero', function (event) {
    var cell = this;
    var data = LoteLinderos.row($(this).parents('tr')).data();
    var enlace = ROOT + 'BienComun/AgregarLindero';
    $.post(enlace,
        {
            CodUnidadLindero: $('#CodigoUnidadCatastral').val(),
            Estatico: Estatico,
